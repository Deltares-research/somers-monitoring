...
sql_create_table = f"""
    CREATE TABLE IF NOT EXISTS b_selection_lulucf.b_{monitoringsjaar}_lulucf(
        ...
        ALTER TABLE b_selection_lulucf.b_{monitoringsjaar}_lulucf OWNER TO somers
"""

cursor.execute(sql_create_table)
connection.commit()

#%% Download needed data from Scheme a

sql_query = f"""SELECT *, ST_AsText(geom) FROM a_input_lulucf.a_{monitoringsjaar}_lulucf  ORDER BY perceel_id"""
...
columns = {'gid': int,
          ...
          'measure': str, # measure type (SSI, PSSI, ref)
          'geometry': object,
          'geometry_wkt': object,
          }

dataframe = pd.DataFrame(records, columns=columns.keys()).astype(columns).set_index('perceel_id')
dataframe['geometry'] = dataframe['geometry_wkt'].apply(shapely.wkt.loads)

#%% Select the rows needed to run SOMERS and upload those columns into the new database table
...
for n, row in dataframe.iterrows(): 
    ...
    surface_level_ahn5 = float(row.ahn5_media) if row.ahn5_media != 8888 else row.ahn3_media # Groningen does not have AHN5 values, so here we keep the AHN3 values
    ...
    postgres_insert_query = f"""
        INSERT INTO b_selection_lulucf.b_{monitoringsjaar}_lulucf (parcel_id, x, y, archetype, surface_level_ahn3, surface_level_ahn5, parcel_width, summer_stage, winter_stage, SSI_distance, SSI_depth,
